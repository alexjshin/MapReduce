...
// Workers call NotifyComplete to tell the master that they've finished a task.
// This lets the master update its count of remaining tasks and task status
func (c *Master) NotifyComplete(args *TaskCompletionArgs, reply *TaskCompletionReply) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Master) RequestTask(args *TaskRequestArgs, reply *TaskRequestReply) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Master) findAvailableReduceTask(workerId string) *ReduceTask {
	// First check for timed out tasks
...
func MakeMaster(files []string, nReduce int) *Master {
	...
	fmt.Printf("Master initialized with %v Map Tasks\n", len(files))
	fmt.Printf("Master initialized with %v Reduce Tasks\n", nReduce)
	c.server()
	return &c
}
